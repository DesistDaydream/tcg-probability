...
$(document).ready(function () {
  ...
  $("body").on(
    "keyup",
    "#deck-size, #hand-size, .card-types-amt, .card-types-min, .card-types-max",
    function (e) {
...
function calculate() {
  ...
  if (!valid) {
    ...
    var color = ["red", "#ff9900", "#ffbf00", "green", "green"][
      Math.floor(chance / 25)
    ]

    $("#percentage").html(
      '<label>You have a <span style="color: ' +
        color +
        '">' +
        chance.toFixed(2) +
        "</span>% chance to open this hand.</label>"
    )
  }
}
